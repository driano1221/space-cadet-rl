...
ordem <- c("Aleatorio", "PPO: score cru", "PPO: score + bonus de sobrevivencia")
d[, agente := factor(agente, levels = ordem)]

d |> summarise(n = n(), mediana = median(score), duracao = mean(duracao),
               .by = agente) |> as.data.frame() |> print(digits = 6)
...
g <- ggplot(d, aes(duracao, score, colour = agente)) +
  ...
  scale_colour_manual(values = c("#8c96c6", "#d95f0e", "#c51b8a"), name = NULL) +
  labs(title = "A recompensa decide o que o agente aprende",
       subtitle = "Losangos = mediana. 40 partidas por agente.",
