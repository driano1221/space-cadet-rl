...
base = SpaceCadetEnv(recompensa=recompensa, max_passos=6000,
                     comprimir=True, bonus_vivo=0.0)
...
print(f"=== {recompensa} | {passos} passos | raiz + normalizacao, SEM bonus de sobrevivencia ===", flush=True)
...
with open(f"resultado3_{recompensa}.csv", "w", newline="") as f:
...
json.dump({"mediana_antes": float(np.median(sa)), "mediana_depois": float(np.median(sd)),
          ...
          open(f"resumo3_{recompensa}.json", "w"), indent=2)
