...
base = SpaceCadetEnv(recompensa=recompensa, max_passos=6000,
                     comprimir=True, bonus_vivo=0.02)
...
print(f"=== {recompensa} | {passos} passos | raiz + normalizacao + ent 0.01 ===", flush=True)
...
with open(f"resultado2_{recompensa}.csv", "w", newline="") as f:
...
json.dump({"mediana_antes": float(np.median(sa)), "mediana_depois": float(np.median(sd)),
          ...
          open(f"resumo2_{recompensa}.json", "w"), indent=2)
